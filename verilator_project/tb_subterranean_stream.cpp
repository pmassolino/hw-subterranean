...
#include <stdio.h>
#include <string.h>
#include "Vsubterranean_stream.h"
#include "verilated.h"
...
#define G_FNAME_AEAD_KAT "../data_tests/LWC_AEAD_KAT_128_128.txt"
...
class Testbench {
    public:
        ...
        Testbench(long long stop_time_value=-1, int skipping_cycles=0) {
            ...
            dut->trace(tfp, 99);
            tfp->open("tb_subterranean_stream.fst");
#else
            tfp = new VerilatedVcdC;
            dut->trace(tfp, 99);
            tfp->open("tb_subterranean_stream.vcd");
#endif
#else
            stop_time_enable = 0;
            stop_time = 0;
        ...
        void send_array(char * message_in, int message_in_length_bytes){
            ...
            for(i = 0; i < (message_in_length_bytes-4);){
                temp_data = 0;
                temp_data_size = 0;
                for(j = 0; (j < 32); j += 8){
            ...
            while((i < message_in_length_bytes) || (z < message_out_length_bytes) || (first_message_sent == 1) || (last_message_sent == 1) || (state_receiving != 0) || (state_sending != 0)){
                if((i < message_in_length_bytes) || (first_message_sent == 1) || (last_message_sent == 1) || (state_sending != 0)){
                    if(state_sending == 0){
                        temp_data = 0;
                        temp_data_size = 0;
                        if(i < (message_in_length_bytes-4)){
                            dut->din_last = 0;
                        }
                        else{
                            dut->din_last = 1;
